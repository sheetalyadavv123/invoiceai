...
export default function PublicInvoice() {
  ...
  useEffect(() => {
    if (!id) return;
    getPublicInvoice(id)
      .then(data => { setInvoice(data); setPaid(data.status === 'paid'); })
  ...
  if (loading) return (
    <div style={s.root}>
      <div style={s.loadingWrap}>
        <div style={s.spinner} />
        <span style={{ color: '#6b7280', fontSize: '14px' }}>Loading invoice...</span>
      </div>
  ...
  if (error || !invoice) return (
    <div style={s.root}>
      <div style={s.errorWrap}>
        ...
        <h2 style={{ color: '#f3e8ff', fontFamily: "'Syne',sans-serif" }}>Invoice not found</h2>
        <p style={{ color: '#6b7280', fontSize: '14px' }}>This invoice may have been deleted or the link is invalid.</p>
      </div>
    </div>
  );
  ...
  const createdAt = new Date(invoice.createdAt).toLocaleDateString('en-IN', {
    day: '2-digit', month: 'long', year: 'numeric'
  });
    ...
    <div style={s.root}>
      ...
      <div style={s.card}>
        ...
        <div style={s.cardHeader}>
          ...
          <div style={{
            ...s.statusBadge,
            background: paid ? 'rgba(16,185,129,0.15)' : invoice.status === 'overdue' ? 'rgba(239,68,68,0.15)' : 'rgba(245,158,11,0.15)',
            color: paid ? '#34d399' : invoice.status === 'overdue' ? '#f87171' : '#fbbf24',
            border: `1px solid ${paid ? 'rgba(16,185,129,0.3)' : invoice.status === 'overdue' ? 'rgba(239,68,68,0.3)' : 'rgba(245,158,11,0.3)'}`,
          }}>
        ...
        <div style={s.metaRow}>
          ...
          <div style={s.metaCard}>
            <div style={s.metaLabel}>Issue date</div>
            <div style={s.metaValue}>{createdAt}</div>
          </div>
          <div style={s.metaCard}>
            <div style={s.metaLabel}>Due date</div>
            <div style={{ ...s.metaValue, color: invoice.status === 'overdue' && !paid ? '#f87171' : '#f3e8ff' }}>
              {dueDate}
            </div>
          </div>
          <div style={s.metaCard}>
            <div style={s.metaLabel}>Amount due</div>
            <div style={{ ...s.metaValue, color: '#a78bfa', fontSize: '20px' }}>
        ...
        {error && (
          <div style={{ background: 'rgba(239,68,68,0.1)', border: '1px solid rgba(239,68,68,0.3)', borderRadius: '10px', color: '#fca5a5', fontSize: '13px', padding: '10px 14px', marginTop: '16px' }}>
            {error}
          </div>
        )}
...
const s: Record<string, React.CSSProperties> = {
  ...
  errorWrap: { display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', height: '100vh', textAlign: 'center' as const },
  ...
  metaValue: { fontSize: '15px', fontWeight: 600, color: '#f3e8ff' },
  partiesRow: { display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1px', background: 'rgba(139,92,246,0.08)', borderBottom: '1px solid rgba(139,92,246,0.1)', margin: '0' },
